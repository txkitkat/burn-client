    ...
    useEffect(() => {
        if (props.map !== undefined && props.seed !== undefined && !sliderRendered) {
            ...
            controlInstance.onAdd = () => {
                const vegetationOverlayLayerDiv = document.createElement("div");
                vegetationOverlayLayerDiv.textContent = LayerNames[0];
                vegetationOverlayLayerDiv.style.textAlign = "center";

                // use Callback function in setValue[] to change the opacity value for a layer
                const vegetationSlider = createSlider({map: props.map, startingValue: "1", setValue: props.setValue[0]});

                div.appendChild(vegetationOverlayLayerDiv);
                div.appendChild(vegetationSlider);
